...
from _revision_ablation_study.fasttext_embedding_init._generate_graph_dict import *
from _revision_ablation_study.fasttext_embedding_init.armadillo import Armadillo
...
def embed_all(graph_dict: dict|str, model_path: str, batch_size: int=64, to_cpu: bool=False, num_workers: int=0) -> dict:
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            embeddings = model(batch[0].to(device))
...
if __name__ == '__main__':
    root_ablation = '/home/francesco.pugnaloni/armadillo_all/ablation_study/fasttext/'
    ...
    graph_dict = generate_graph_dictionary(table_dict_path=table_dict, embedding_generation_method='fasttext', save_graph_dict=False)
    end_dict = time.time()
