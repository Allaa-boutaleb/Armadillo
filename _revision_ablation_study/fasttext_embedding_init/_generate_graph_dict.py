from _revision_ablation_study.fasttext_embedding_init.graph import *
from tqdm import tqdm
import sys
import time

def generate_graph_dictionary(table_dict_path: str | dict, out_path: str=None, embedding_generation_method: str='fasttext', save_graph_dict: bool=True) -> dict:
    ...
    for k in tqdm(table_dict.keys()):
        try:
            if embedding_generation_method == 'sha256':
                ...
                out[k] = Graph(table_dict[k], k, embedding_buffer_type=embedding_generation_method, preprocess_string_token=string_token_preprocessor, embedding_buffer=embedding_buffer, string_preprocess_operations=string_token_preprocessor, token_length_limit=1000)
        except:
            out[k] = None
