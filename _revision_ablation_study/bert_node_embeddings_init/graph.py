...
import math
from _revision_ablation_study.bert_node_embeddings_init.node_embeddings import *
...
class Graph:
    ...
    def __init__(self,  df: pd.DataFrame, table_name: str, embedding_buffer: Embedding_buffer=None,
            ...
            drop_na: bool=False, verbose: bool=False, 
            merge_nodes_same_value: bool=False) -> None:
            embedding_buffer.load_table(df)
    ...
    def init_sha256(self,  df: pd.DataFrame, table_name: str,  embedding_buffer: Embedding_buffer=None,
        ...
        if embedding_buffer == None:
            raise Exception('You must provide a BertEmbeddingBuffer')
        ...
        n_rows, n_columns = embedding_buffer.get_max_rows_columns(df.shape[0], df.shape[1])
...
if __name__ == '__main__':
    df = pd.DataFrame({'b': [4, 5, 6],'c': [7, 2, 1],'a': [' '.join(['cat' for _ in range(1_000)]),'geronimo, stilton, e#paperino', 'magic the gathering 2']})
    emb = BertEmbeddingBuffer()
    g1 = Graph(df, 'table_name', emb)
    g2 = Graph(df, 'table_name', emb)
    df = pd.DataFrame({'b': [4, 5],'c': [7, 2],'a': ['cat','geronimo']})
    g3 = Graph(df, 'table_name', emb)
    pass