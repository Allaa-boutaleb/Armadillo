from _revision_ablation_study.bert_node_embeddings_init.graph import *
...
def generate_graph_dictionary(table_dict_path: str | dict, out_path: str=None, embedding_generation_method: str='sha256', save_graph_dict: bool=True) -> dict:
    ...
    embedding_buffer = BertEmbeddingBuffer()
    ...
    for k in tqdm(table_dict.keys()):
        try:
            out[k] = Graph(table_dict[k], k, embedding_buffer=embedding_buffer, merge_nodes_same_value=False)
        except:
            out[k] = None
