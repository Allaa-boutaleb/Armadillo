...
import pickle
import pandas as pd
from _revision_ablation_study.absolute_value_cosine_similarity.graph import *
...
import wandb
import warnings
from _revision_ablation_study.absolute_value_cosine_similarity.my_constants import *
...
def train_epoch(model: Armadillo, train_dataloader: DataLoader, optimizer: torch.optim.Optimizer, criterion: nn.MSELoss, device: str) -> torch.Tensor:
    ...
    for batch in train_dataloader:
        ...
        predictions = torch.abs(F.cosine_similarity(emb_l, emb_r, dim=1))
...
def model_inference(model: Armadillo, data_loader: DataLoader, device: str) -> set:
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            logits = torch.abs(F.cosine_similarity(emb_l, emb_r, dim=1))
