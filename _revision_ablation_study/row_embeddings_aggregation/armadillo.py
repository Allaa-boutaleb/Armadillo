...
import pickle
import pandas as pd
from _revision_ablation_study.row_embeddings_aggregation.graph import *
...
import wandb
import warnings
from _revision_ablation_study.row_embeddings_aggregation.my_constants import *
...
class Armadillo(nn.Module):
    ...
    def forward(self, b: Batch, start: Batch, end:Batch, size:Batch) -> torch.tensor:
        ...
        for i in range(len(size)):
            ...
            id_end = offset + end[i]
            try:
                out = torch.cat((out, out_gin[id_start:id_end].mean(axis=0).unsqueeze(0)), dim=0)
            except:
                out = out_gin[id_start:id_end].mean(axis=0).unsqueeze(0)
            offset += size[i]
        return out
