...
class TURL_embedding_generator:
    def __init__(self, data_dir: str="turl/datasets/turl_datasets/",
                 ...
                 max_lines: int=300, config_name: str="experiments/ablation_modified/code_TURL/TURL/configs/table-base-config_v2.json",
                 sampling_size: int=10, gpu_num: str='0', max_t_exec: int=60) -> None:
        #Model variables
        self.max_lines = max_lines
        self.max_t_exec = max_t_exec
    ...
    def prepare_dataframe(self, table: str | pd.DataFrame, max_lines: int) -> pd.DataFrame:
    ...
    def embed(self, file: str | pd.DataFrame) -> dict:
    ...
    def __call__(self, file: str | pd.DataFrame) -> torch.Tensor:
        try:
            with timeout(self.max_t_exec, exception=TimeoutError):
                emb = self.embed(file)['file_embedding']
        except TimeoutError:
